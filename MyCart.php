...
$d= date('Y-m-d h:i:s', time());
        ...
        if(isset($_SESSION['customer'])):
          ...
          $calling = $mysqli->query("select * from customer where c_email='$log'");
...
$id=date('Ymdhis');
        ...
        foreach ($array as $key => $value){
                                        $mysqli = new mysqli("localhost","root","","foodstuff");
					$insert = $mysqli->query("INSERT INTO orders (invoice_no,c_id,p_id,v_email,qty) values ('$id','$value[0]','$value[1]','$value[2]','$value[4]')");
				}
				$del = $mysqli->query("DELETE FROM cart where cart.c_id='".$data['c_id']."'");
