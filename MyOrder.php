...
<html lang="en" dir="ltr">
  ...
  <body style="background:#f1f3f6">
      ...
      if(isset($_SESSION['customer'])):
        ...
        $query = $mysqli->query("SELECT DISTINCT invoice_no from orders  inner join customer
      ...
      while($order_data = mysqli_fetch_array($query)): ?>
        ...
        <div class="col-lg-12">
          <div class="card mb-4">
                  ...
                  <?php
                    $mysqli = new mysqli("localhost","root","","foodstuff");
                    $order = $mysqli->query("SELECT * from orders inner join customer
                    on orders.c_id=customer.c_id
                    inner join product
                      on orders.p_id=product.p_id
                    inner join vendor
                                 ...
                                 if($data['order_status']=='Active' || $data['order_status']=='Delivered'):
                                       ...
                                       //echo date('Y-m-d h:i:s', time());
                                        if( date('Y-m-d h:i:s', time())>$del_time):
                                          $o_id=$data['o_id'];
                                          $mysqli = new mysqli("localhost","root","","foodstuff");
                                          $delevered= $mysqli->query( "UPDATE orders
                                            set
