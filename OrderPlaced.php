  ...
  <body style="background:#f1f3f6">
      ...
      <?php
        if(isset($_SESSION['customer'])):
          ...
          $c_query = $mysqli->query("SELECT * FROM customer WHERE c_email='$log'");
          $c_row=mysqli_fetch_array($c_query);

          $p_query = $mysqli->query("SELECT * FROM product inner JOIN vendor on  vendor.v_email=product.v_email WHERE p_id='$p_id'");
          $p_row=mysqli_fetch_array($p_query);
          $o_query = $mysqli->query("SELECT * FROM orders WHERE p_id='$p_id'");
