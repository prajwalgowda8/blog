...
if(isset($_POST["add_to_cart"]))
{
		      ...
		      $duplicate=$mysqli->query("SELECT * FROM cart WHERE p_id='$p_id' AND c_id='$c_id'");
		         if(mysqli_num_rows($duplicate)==0){
                           $mysqli = new mysqli("localhost","root","","foodstuff");  
		           $insert = $mysqli->query("INSERT INTO cart (c_id,p_id,v_email,qty) values ('$c_id','$p_id','$v_email','$qty')");
					      }
          ...
          <div class="row">
              <div class="col-4 col-lg-2 col-md-2  mt-4">
                ...
                <div class="list-group shadow" style="padding:0px">
                      ...
                      <?php
                          $mysqli = new mysqli("localhost","root","","foodstuff");
                          $cat = $mysqli->query("SELECT * FROM category");//stores all categories in $cat variable
              ...
              <?php
                $mysqli = new mysqli("localhost","root","","foodstuff");
                $e= mysqli_num_rows($mysqli->query("SELECT * from product where p_status='Available'"));
              ?>
              ...
              <div class="row">
                ...
                /*if the user will search by name of product then this if will be executed and filterred data will be shown*/
                  if(isset($_GET['find'])){
                      $search = $_GET['search'];
                      $mysqli = new mysqli("localhost","root","","foodstuff");
                      $calling = $mysqli->query("SELECT * FROM product WHERE p_name LIKE '%$search%' AND p_status='Available'");
                    }
                ...
                /*if the user will search category wise then this elseif will be executed and filterred data will be shown*/
                  elseif(isset($_GET['cat'])){
                      $cat = $_GET['cat'];
                      $mysqli = new mysqli("localhost","root","","foodstuff");
                      $calling = $mysqli->query("SELECT * FROM product WHERE cat_title='$cat' AND p_status='Available' ");
                    }

              /* if the user will directly go to the products page then this else will be executed without filteration*/
                  else{
                      $mysqli = new mysqli("localhost","root","","foodstuff");
                      $calling = $mysqli->query("SELECT * FROM product WHERE p_status='Available'");  /*collects all data from product table*/
                    }
