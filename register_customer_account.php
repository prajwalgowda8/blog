...
<head>
 ...
 if(isset($_POST['c_register'])){
   ...
   $duplicate=$mysqli->query("SELECT * FROM customer WHERE c_email='$c_email'");
   if(mysqli_num_rows($duplicate)>0){
     	echo "<script>alert('This Email Id is Already Existed')</script>";
   }
   else{
     $mysqli = new mysqli("localhost","root","","foodstuff");
     $query = $mysqli->query("INSERT INTO customer (c_f_name,c_phn,c_email,c_add,c_pin,c_pass,c_img,c_L_name,c_age,c_gen,c_city,c_state) value('$c_f_name','$c_phn','$c_email','$c_add','$c_pin','$c_pass','$c_img','$c_L_name','$c_age','$c_gen','$c_city','$c_state')");
