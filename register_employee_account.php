...
<head>
 ...
 if(isset($_POST['emp_register'])){
   ...
   $duplicate=$mysqli->query("SELECT * FROM employee WHERE emp_email='$emp_email'");
   if(mysqli_num_rows($duplicate)>0){
       echo "<script>alert('This Email Id is Already Existed')</script>";
   }
   else{
     $mysqli = new mysqli("localhost","root","","foodstuff");
     $query = $mysqli->query("INSERT INTO employee (emp_f_name,emp_phn,emp_email,emp_add,emp_pin,emp_pass,emp_img,emp_L_name,emp_age,emp_gen,emp_city,emp_state) value('$emp_f_name','$emp_phn','$emp_email','$emp_add','$emp_pin','$emp_pass','$emp_img','$emp_L_name','$emp_age','$emp_gen','$emp_city','$emp_state')");
