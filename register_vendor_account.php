...
<head>
 ...
 if(isset($_POST['v_register'])){
   ...
   $duplicate=$mysqli->query("SELECT * FROM vendor WHERE v_email='$v_email'");
      if(mysqli_num_rows($duplicate)>0){
       echo "<script>alert('This Email Id  Already Exist')</script>";
   }
   else{
   $mysqli = new mysqli("localhost","root","","foodstuff");
   $query = $mysqli->query("INSERT INTO vendor (v_f_name,v_phn,v_email,v_add,v_pin,v_pass,v_img,v_L_name,v_age,v_gen,v_city,v_state) value('$v_f_name','$v_phn','$v_email','$v_add','$v_pin','$v_pass','$v_img','$v_L_name','$v_age','$v_gen','$v_city','$v_state')");
